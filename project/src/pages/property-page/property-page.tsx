...
function PropertyPage({offers, reviews, authorizationStatus, favoritesCount, onComment}: PlaceCardProps): JSX.Element {
  ...
  const offer: Offer = offers.filter((el) => el.id === Number(params.id))[0];
  const { isPremium, price, isFavorite, rating, type, title, bedrooms, maxAdults, goods, host } = offer;
  const placeReviews: Reviews = reviews.filter((el) => el.id === Number(params.id));
  console.log(offers);
  const placesNearby: Offers = offers
      ...
      <main className="page__main page__main--property">
              ...
              {offer.images.map((el, idx) => {
                ...
                return (
                  <div key={key} className="property__image-wrapper">
                    <img className="property__image" src={`img/${el}`} alt="Photo studio" />
                  </div>
                );
              })}
            </div>
        ...
        <div className="container">
          <section className="near-places places">
            ...
            <PlaceCardList offers={placesNearby} nearby={true} />
          </section>
        </div>
      </main>
    </div>
